...
def api_auth():
    ...
    if token == ARIA_DASHBOARD_TOKEN:
        return jsonify({"token": token, "plan": "free", "name": "Public User"})
    elif token == "PREMIUM12345":
        return jsonify({"token": token, "plan": "premium", "name": "Admin Premium", "expiry": "2099-12-31"})
    else:
...
def api_news():
    ...
    url = f"https://newsapi.org/v2/top-headlines?category=business&language=fr&apiKey={NEWSAPI_KEY}"
    try:
        ...
        for i, art in enumerate(articles):
            source_name = art.get("source", {}).get("name", "Actualité")
            if source_name == "NewsAPI" or not source_name:
                source_name = "Actualité"
            formatted_news.append({
                "id": i,
                "title": art.get("title", ""),
                "source": source_name,
                "time": art.get("publishedAt", "")[:10],
                "url": art.get("url", ""),
                "description": art.get("description", "")
            })
...
def api_news_analysis():
    ...
    # Génération d'une analyse via l'IA Groq
    prompt = "Fais une analyse financière courte en français d'une actualité économique majeure récente. Structure ta réponse en JSON avec les clés exactes: summary, impact, opportunity, risks, horizon."
    try:
        ...
        if start != -1 and end != -1:
            ...
            return jsonify({
                "summary": "Analyse non disponible pour le moment.",
                "impact": "Inconnu",
                "opportunity": "Aucune",
                "risks": "Non évalués",
                "horizon": "Court terme"
...
def api_signal():
    return jsonify({
        ...
        "target": 68000,
        "stop": 61000,
        "reason": "Signal généré par ARIA suite à une analyse technique.",
        "risk": "Medium"
    })
...
def api_signals():
    return jsonify([
        {"asset": "BTC", "direction": "BUY", "conviction": 85, "target": 68000, "stop": 61000, "reason": "Tendance haussière confirmée", "risk": "Medium"},
        {"asset": "ETH", "direction": "SHORT", "conviction": 60, "target": 3200, "stop": 3600, "reason": "Résistance majeure atteinte", "risk": "High"}
    ])

@app.route('/api/rsi', methods=['GET'])
def api_rsi():
    return jsonify([
        {"asset": "BTC", "value": 65, "price": 64500},
        {"asset": "ETH", "value": 45, "price": 3400}
    ])
...
def api_score():
    return jsonify({
        "score": 78,
        "trend": "Bullish",
        "factors": ["Forte demande institutionnelle", "Baisse de l'inflation"]
    })
...
def api_gem():
    return jsonify({
        "asset": "SOL",
        "reason": "Activité réseau en forte hausse, sous-évalué par rapport à ETH.",
        "conviction": 88,
        "horizon": "1-3 mois"
    })
...
def api_lessons():
    return jsonify({
        "title": "RSI — Lire le Relative Strength Index",
        "content": "Le RSI est un oscillateur de momentum qui mesure la vitesse et le changement des mouvements de prix. Il oscille entre 0 et 100.",
        "example": "Un RSI > 70 indique un actif potentiellement suracheté. Un RSI < 30 indique un actif survendu."
    })
...
@app.route('/api/alerts/del', methods=['POST', 'DELETE'])
def api_alerts_del():
    return jsonify({"status": "success"})

@app.route('/api/trade/buy', methods=['POST'])
def api_trade_buy():
    return jsonify({"status": "success", "message": "Achat simulé avec succès"})

@app.route('/api/trade/sell', methods=['POST'])
def api_trade_sell():
    return jsonify({"status": "success", "message": "Vente simulée avec succès"})

@app.route('/api/copytrade/toggle', methods=['POST'])
def api_copytrade_toggle():
    return jsonify({"status": "success", "copyTrading": True})
